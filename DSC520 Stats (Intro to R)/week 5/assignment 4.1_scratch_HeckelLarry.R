...
library(data.table)
library(purrr)
#read the files together into a single data frame
#this code assumes that these files are in the working
#directory and are the only *.txt files there.
comboDonation <- list.files(pattern = "*.txt") %>% map_df(~fread(., stringsAsFactors = FALSE))
view(comboDonation)
str(comboDonation)
